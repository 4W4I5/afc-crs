...
You should include all functions of the application code that appear in the crash log (but are not fuzzer harness code).
...
def main():
    ...
    with tracer.start_as_current_span("patch_delta") as span:
        span.set_attribute("crs.action.category", "patch_generation")
        span.set_attribute("crs.action.name", f"patching_delta_scan_advanced_strategy_v1")
        span.set_attribute("service.name", "patch1_delta")
