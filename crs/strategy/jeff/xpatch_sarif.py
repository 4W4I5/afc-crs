...
#!/usr/bin/env python3
"""
XPatch Strategy: LLM-guided patch generation w/ sarif info
"""
...
def get_fallback_model(current_model, tried_models):
    ...
    fallback_chains = {
        ...
        CLAUDE_MODEL: [CLAUDE_MODEL_SONNET_45,OPENAI_MODEL, OPENAI_MODEL_O3, GEMINI_MODEL_PRO_25],   
...
def setup_logging(fuzzer_name):
    ...
    log_file = os.path.join(LOG_DIR, f"xpatch_sarif_{fuzzer_name}_{patch_status}_{scan_type}_{timestamp}.log")
...
def submit_patch_to_endpoint(log_file, pov_signature, sarif_id, patch_diff):
    """
                ...
                while time.time() < deadline:
                    try:
                        ...
                        if status == "passed":
                            ...
                            try:
                                with open("xpatch_id.txt", "w") as f:
                                    f.write(patch_id_x)
                            except Exception as e:
    ...
    for target in target_functions:
        ...
        if os.path.exists(potential_file):
            ...
            for rel in GLOBAL_RELEVANT_SOURCE_FILES:
                ...
                if (abs_path.endswith(extension) or abs_path.endswith(".in")) and os.path.exists(abs_path):
            ...
            for root, dirs, files in os.walk(project_src_dir):
                for file in files:
                    if (file.endswith(extension) or file.endswith(".in")) and not file.startswith("Crash_"):
...
def apply_patch(log_file, patch_code_dict, project_dir, project_src_dir, project_name, sanitizer, language, patch_id):
    """
...
def run_fuzzer_for_xpatch(log_file, fuzzer_path, project_dir, focus, sanitizer, project_name, seed_corpus_dir):
    ...
    try:
        ...
        fuzzer_timeout = 25  # 25 seconds for the fuzzer
        subprocess_timeout = 30  # 30 seconds for the subprocess
        ...
        log_message(log_file, f"[XPATCH-SARIF run_fuzzer_for_xpatch-sarif] Running Docker command: {' '.join(docker_cmd)}")
        ...
        crash_detected = (process.returncode != 0)
        log_message(log_file, f"Fuzzer exited with returncode: {process.returncode}")            
        if crash_detected:
            log_message(log_file, f"Fuzzer crashed with fuzzer_output: {fuzzer_output}")
                
        return crash_detected, fuzzer_output
    
    except Exception as e:
        log_message(log_file, f"Error running fuzzer: {str(e)}")
        return False, str(e)

def confirm_patch_validity_with_ai(
    log_file,
    sarif_vulnerability_desc: str,
    functions_metadata_str: str,
    patch_code_dict: Dict[str, str],
    patch_diff: str,
) -> bool:
    """
    Ask an LLM for a binary judgement on whether the proposed patch fully
    fixes the vulnerability without breaking anything else.  The model must
    respond with the exact string 'ABSOLUTELY VALID PATCH' (case-insensitive)
    to be considered a positive verdict.
    """

    # Helper – keep any individual text block small so the overall prompt fits
    def _clip(txt: str, max_lines: int = 2000) -> str:
        return truncate_output(txt, max_lines=max_lines)

    # --------  Assemble prompt -------------------------------------------------
    system_prompt = (
        "You are a world-leading security engineer.  Your ONLY task is to decide "
        "whether the supplied patch 100 % conclusively fixes the vulnerability "
        "in the original functions without introducing new issues.  Output exactly one of:\n"
        "  • ABSOLUTELY VALID PATCH\n"
        "  • NO"
    )

    # Pretty-print the generated patch code (one file per entry)
    pretty_patch_code = "\n\n".join(
        f"# {fname}\n```diff\n{_clip(code, 2000)}\n```"
        for fname, code in patch_code_dict.items()
    )

    user_prompt = (
        "===== ORIGINAL FUNCTIONS =====\n"
        f"{_clip(functions_metadata_str)}\n\n"
        "===== VULNERABILITY DESCRIPTION =====\n"
        f"{_clip(sarif_vulnerability_desc)}\n\n"
        "===== PATCH CODE =====\n"
        f"{pretty_patch_code}\n\n"
        "===== TEXTUAL DIFF OF PATCH =====\n"
        f"{_clip(patch_diff)}\n\n"
        "Please answer with EXACTLY the phrase 'ABSOLUTELY VALID PATCH' "
        "if – and only if – you are 100 % certain the patch is correct.  "
        "Otherwise reply 'NO'."
    )
    ...
    model = CLAUDE_MODEL
    answer, success = call_llm(log_file, messages, model)
    if success:
        answer_clean = answer.strip().upper()
        if answer_clean.startswith("ABSOLUTELY VALID PATCH"):
            log_message(log_file, f"{model}: patch confirmed valid ✔")
            return True
        else:
            log_message(
                log_file,
                f"{model}: LLM did NOT confirm patch (answer: {answer_clean[:80]}...)",
            )
...
def format_sarif_vulnerability_description(sarif_broadcast: Dict[str, Any]) -> str:
    """
    Build a concise textual description of the vulnerability described in
    a SARIF broadcast so it can be embedded directly in the LLM prompt.

    The function extracts the rule-ID, severity, human message and the
    first physical location for every result it can find.  If the
    `metadata` section of the broadcast already carries a free-form
    description, that is included at the top.
    """
    sarif_data = sarif_broadcast.get("sarif", {}) or {}
    meta       = sarif_broadcast.get("metadata", {}) or {}

    def _severity(obj: Dict[str, Any]) -> str:
        return str(
            obj.get("level") or
            obj.get("properties", {}).get("severity") or
            obj.get("properties", {}).get("securitySeverity") or
            "UNKNOWN"
        ).upper()

    pieces: List[str] = []

    # Prefer a top-level, human-written description if available
    if meta.get("description"):
        pieces.append(meta["description"].strip())
    ...
    for run in runs:
        ...
        for res in run.get("results", []):
            rule_id = res.get("ruleId") or res.get("rule_id") or "N/A"
            rule    = rule_lookup.get(rule_id, {})

            message = (
                res.get("message", {}).get("text") or
                rule.get("fullDescription", {}).get("text") or
                rule.get("shortDescription", {}).get("text") or
                ""
            ).strip()

            severity = _severity(res) if _severity(res) != "UNKNOWN" else _severity(rule)

            loc_str = ""
            if res.get("locations"):
                phys = res["locations"][0].get("physicalLocation", {})
                uri  = phys.get("artifactLocation", {}).get("uri", "")
                region = phys.get("region", {})
                start  = region.get("startLine", 0)
                end    = region.get("endLine", start)
                if uri:
                    loc_str = f"{uri}:{start}-{end}"

            summary = f"* Rule: {rule_id} | Severity: {severity}"
            if loc_str:
                summary += f" | Location: {loc_str}"
            if message:
                summary += f"\n  Description: {message}"
    ...
    if not pieces:
        return "No detailed vulnerability information was available in the SARIF report."

    return "\n\n".join(pieces)

def doPatch(log_file, base_dir, fuzzer_path, project_dir, project_name, focus, sanitizer, language, sarif_broadcast, model_name, attempt_dir):
    """
    ...
    fuzzer_name = os.path.basename(fuzzer_path)
    seed_corpus_dir = os.path.join(base_dir, f"{fuzzer_name}_seed_corpus")
    ...
    if len(sarif_targets) == 0:
        ...
        log_message(log_file, f"find_sarif_target_functions_with_llm sarif_targets: {sarif_targets}")
            ...
            try:
                for func in sarif_targets:
                    ...
                    if int(start_line) > 0 and int(end_line) > 0:
                        start_line_x = start_line
                        end_line_x = end_line

            except Exception as e:
                log_message(log_file, f"Error checking sarif_targets: {str(e)}")
        
        log_message(log_file, f"start_line_x: {start_line_x}")
        log_message(log_file, f"end_line_x: {end_line_x}")
    ...
    if not function_metadata:
        log_message(log_file, "Could not find metadata for target functions, patching failed!")
        return False, patch_id
    else:
    ...
    sarif_vulnerability_desc = format_sarif_vulnerability_description(sarif_broadcast)
    ...
    initial_msg = INITIAL_PATCH_TEMPLATE.format(
        sarif_vulnerability_desc=sarif_vulnerability_desc,
        functions_metadata_str=functions_metadata_str
    )
    
    log_message(log_file, f"[XPATCH-SARIF] doPatch {model_name} initial_msg:\n{initial_msg}")
    ...
    # MAX_ITERATIONS = 2
    xpatch_failed_first_time = True
    for iteration in range(1, MAX_ITERATIONS + 1):            
        log_message(log_file, f"XPatch-Sarif iteration {iteration} with model {model_name}")
        ...
        success, stdout, stderr = apply_patch(log_file, patch_code_dict, project_dir, project_src_dir, project_name, sanitizer, language, patch_id)
...
"""
        ...
        crash_detected, fuzzer_output = run_fuzzer_for_xpatch(log_file, fuzzer_path, project_dir, focus,sanitizer,project_name, seed_corpus_dir)
        ...
        if not crash_detected:
            ...
            valid_patch_confirmed = confirm_patch_validity_with_ai(log_file, sarif_vulnerability_desc, functions_metadata_str, patch_code_dict, patch_diff)   
            if valid_patch_confirmed:
            ...
            log_message(log_file, "XPatch-Sarif Failed!")
            if not xpatch_failed_first_time:
                exit(0)
            
            xpatch_failed_first_time = False
...
def doPatchUntilSuccess(log_file, fuzzer_path, project_dir, project_name, focus, sanitizer, language, sarif_broadcast):
    """
    ...
    Returns:
        bool: True if patching was successful, False otherwise
    """
    log_message(log_file, f"Starting xpatching sarif process...")
    ...
    if "patch_workspace" in base_dir:
        # Keep everything *before* the first “patch_workspace”
        base_dir = base_dir.split("patch_workspace")[0].rstrip(os.sep)
    # Fall-back to the original dir if the above produced an empty string
    if not base_dir:
        base_dir = project_dir
    ...
    for iteration in range(1, 2 + 1):        
        ...
        available_models = [CLAUDE_MODEL_OPUS_4, OPENAI_MODEL_O3]
        ...
        attempt_log = os.path.join(attempt_dir, "xpatch_log.txt")
        with open(attempt_log, "w") as f:
            f.write(f"XPatch attempt {iteration} using model {model_name}\n")
        
        # Attempt to patch using the primary POV
        patch_success, patch_id = doPatch(attempt_log, base_dir, fuzzer_path, project_dir, project_name, focus, sanitizer, language, sarif_broadcast, model_name, attempt_dir)
    ...
    log_message(log_file, f"Failed to xpatch-sarif vulnerability...")
    return False
...
def main():
    parser = argparse.ArgumentParser(description="XPatching SARIF Strategy: LLM-guided Patch Generation")
    ...
    with tracer.start_as_current_span("xpatch_sarif") as span:
        span.set_attribute("crs.action.category", "patch_generation")
        span.set_attribute("crs.action.name", f"xpatching_sarif_strategy")
        span.set_attribute("service.name", "xpatch_sarif")
        span.set_attribute("fuzzer.path", f"{fuzzer_path}")
        if task_detail:
            for key, value in task_detail["metadata"].items():
                span.set_attribute(key, value)   
        try:
            patch_success = doPatchUntilSuccess(log_file,fuzzer_path,project_dir,project_name,focus,sanitizer, language,sarif_broadcast)
        except Exception as e:
