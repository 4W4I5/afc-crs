...
def doPatch_full(log_file, fuzzer_path, project_dir, project_name, focus, sanitizer, language, pov_metadata, model_name, attempt_dir):
    ...
    initial_msg = INITIAL_PATCH_TEMPLATE.format(
        crash_log=crash_log,
        vul_analysis=first_assistant_message,
...
def main():
    ...
    with tracer.start_as_current_span("patch_full") as span:
        span.set_attribute("crs.action.category", "patch_generation")
        span.set_attribute("crs.action.name", f"patching_full_scan_advanced_strategy_v3")
        span.set_attribute("service.name", "patch3_full")
