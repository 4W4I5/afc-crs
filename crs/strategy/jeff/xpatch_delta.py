...
#!/usr/bin/env python3
"""
XPatch Strategy: LLM-guided patch generation for delta scans
"""
...
def confirm_patch_validity_with_ai(
    ...
    system_prompt = (
        ...
        "introduced by the commit diff without introducing new issues.  Output exactly one of:\n"
...
def doPatch(log_file, base_dir, fuzzer_path, project_dir, project_name, focus, sanitizer, language, model_name, attempt_dir):
    ...
    diff_functions = extract_diff_functions_using_funtarget(project_src_dir,project_dir)
...
"""
        ...
        if not crash_detected:
            ...
            valid_patch_confirmed = confirm_patch_validity_with_ai(log_file, commit_diff, functions_metadata_str, patch_code_dict, patch_diff)   
...
def main():
    parser = argparse.ArgumentParser(description="XPatching Strategy Delta Scan: LLM-guided Patch Generation")
    ...
    with tracer.start_as_current_span("xpatch_delta") as span:
        span.set_attribute("crs.action.category", "patch_generation")
        span.set_attribute("crs.action.name", f"xpatching_delta_strategy")
        span.set_attribute("service.name", "xpatch_delta")
