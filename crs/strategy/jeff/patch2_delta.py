...
def doPatch(log_file, fuzzer_path, project_dir, project_name, focus, language, pov_metadata, model_name, attempt_dir):
    """
...
def doPatchUntilSuccess(log_file, fuzzer_path, project_dir, project_name, focus, language, pov_metadata=None):
    ...
    for iteration in range(1, MAX_ITERATIONS + 1):
        ...
        patch_success, patch_id = doPatch(attempt_log, fuzzer_path, project_dir, project_name, focus, language, primary_pov, model_name, attempt_dir)
...
def main():
    ...
    with tracer.start_as_current_span("patch_delta") as span:
        span.set_attribute("crs.action.category", "patch_generation")
        span.set_attribute("crs.action.name", f"patching_delta_scan_advanced_strategy_v2")
        span.set_attribute("service.name", "patch2_delta")
