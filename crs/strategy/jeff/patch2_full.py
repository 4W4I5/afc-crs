...
def run_fuzzer_with_input_for_c_coverage(
    ...
    try:
        ...
        cov_fuzzer_path = os.path.join(fuzzer_dir,"xp0",fuzzer_name)
        host_cov_fuzzer   = os.path.join(out_dir, fuzzer_name)
        shutil.copy(cov_fuzzer_path, host_cov_fuzzer)
...
def doPatch_full(log_file, fuzzer_path, project_dir, project_name, focus, language, pov_metadata, model_name, attempt_dir):
    """
...
"""
        ...
        crash_detected, fuzzer_output = run_fuzzer_with_input(log_file, fuzzer_path, project_dir, focus, test_blob_file, is_c_project, patch_id)
...
def validate_patch_against_all_povs(log_file, fuzzer_path, project_dir, focus, all_povs, language='c', patch_id=''):
    ...
    for i, pov_metadata in enumerate(all_povs):
        ...
        is_c_project = language.startswith('c')
        crash_detected, fuzzer_output = run_fuzzer_with_input(log_file, fuzzer_path, project_dir, focus, blob_path, is_c_project, patch_id)
        fuzzer_output = filter_instrumented_lines(fuzzer_output)
        if crash_detected:
...
def doPatchUntilSuccess(log_file, fuzzer_path, project_dir, project_name, focus, language, pov_metadata=None):
    """
    ...
    for iteration in range(1, MAX_ITERATIONS + 1):
        ...
        patch_success, patch_id = doPatch_full(attempt_log, fuzzer_path, project_dir, project_name, focus, language, primary_pov, model_name, attempt_dir)
        ...
        if patch_success:
            ...
            all_povs_blocked = validate_patch_against_all_povs(attempt_log, fuzzer_path, project_dir, focus, all_povs, language, patch_id)
...
def main():
    ...
    with tracer.start_as_current_span("patch_full") as span:
        span.set_attribute("crs.action.category", "patch_generation")
        span.set_attribute("crs.action.name", f"patching_full_scan_advanced_strategy_v2")
        span.set_attribute("service.name", "patch2_full")
