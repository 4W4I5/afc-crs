...
#!/usr/bin/env python3
"""
XPatch Strategy: LLM-guided patch generation for full scans
"""
...
def setup_logging(fuzzer_name):
    ...
    patch_status = "xpatch_only"
    scan_type = "full_scan"
    
    timestamp = int(time.time())
    log_file = os.path.join(LOG_DIR, f"xpatch_{fuzzer_name}_{patch_status}_{scan_type}_{timestamp}.log")
    
    # Log initial configuration
    with open(log_file, "w") as f:
        f.write(f"XPatching Strategy: Full Scan\n")
...
def run_fuzzer_for_xpatch(log_file, fuzzer_path, project_dir, focus, sanitizer, project_name, seed_corpus_dir):
    ...
    try:
        ...
        log_message(log_file, f"[XPATCH run_fuzzer_for_xpatch] Running Docker command: {' '.join(docker_cmd)}")
...
def doPatch(log_file, base_dir, fuzzer_path, project_dir, project_name, focus, sanitizer, language, model_name, attempt_dir):
    """
    ...
    log_message(
        log_file,
        f"Targeting {len(top_candidates)} high-score functions: {top_candidates}",
    )
    ...
    # b) Fetch metadata for those functions (file path, lines, etc.)
    function_metadata = {}
    for item in top_candidates:
        ...
        abs_path = item.get("file") or item.get("FilePath", "")
        # Map the original file into the *patched* workspace tree
        ...
        if focus:
            ...
            if idx != -1:
                ...
                # look for “/<focus>-<something>/…”
                marker = f"/{focus}-"
                idx = abs_path.find(marker)
                if idx != -1:
                    # drop “…/<focus>-<something>/”
                    tail = abs_path.split(marker, 1)[1]
                    # remove the “-<something>/” part
                    tail = tail.split("/", 1)[1] if "/" in tail else tail
                    rel_path = tail
        else:
            rel_path = f"{name}.c"

        file_path = os.path.join(project_src_dir, rel_path)

        log_message(log_file, f"abs_path: {abs_path}")
        log_message(log_file, f"rel_path: {rel_path}")
        log_message(log_file, f"file_path: {file_path}")

        source = (
            item.get("body")
            or item.get("Body")
            or item.get("sourceCode")
            or item.get("SourceCode")
            or ""
        )
        # Accept either "StartLine"/"startLine"
        start_line = int(item.get("startLine") or item.get("StartLine") or 1)
        # Accept either "EndLine"/"endLine"
        end_line = int(item.get("endLine") or item.get("EndLine") or 0) or \
                    (start_line + source.count("\n"))

        # source = item.get("SourceCode", "")
        # start_line = int(item.get("StartLine", 1))
        # end_line = int(item.get("EndLine", 0)) or \
        #            (start_line + source.count("\n"))
        ...
        GLOBAL_RELEVANT_SOURCE_FILES.add(file_path)
    ...
    log_message(log_file, f"doXPatch {model_name} initial_msg:\n{initial_msg}")
    ...
    for iteration in range(1, MAX_ITERATIONS + 1):
        log_message(log_file, f"XPatch iteration {iteration} with model {model_name}")
...
"""
        ...
        if not crash_detected:
            ...
            valid_patch_confirmed = confirm_patch_validity_with_ai(log_file, functions_metadata_str, patch_code_dict, patch_diff)   
            if valid_patch_confirmed:
                # Submit PATCH to endpoint
                pov_signature = f"xpatch-{project_name}"
            ...
            log_message(log_file, "XPatch Failed!")
...
def doPatchUntilSuccess(log_file, fuzzer_path, project_dir, project_name, focus, sanitizer, language):
    """
    ...
    Returns:
        bool: True if patching was successful, False otherwise
    """
    log_message(log_file, f"Starting xpatching process...")
    ...
    for iteration in range(1, 2 + 1):        
        ...
        patch_success, patch_id = doPatch(attempt_log, base_dir, fuzzer_path, project_dir, project_name, focus, sanitizer, language, model_name, attempt_dir)
    ...
    log_message(log_file, f"Failed to xpatch vulnerability...")
    return False
...
def main():
    parser = argparse.ArgumentParser(description="XPatching Strategy Full Scan: LLM-guided Patch Generation")
    ...
    with tracer.start_as_current_span("xpatch_full") as span:
        span.set_attribute("crs.action.category", "patch_generation")
        span.set_attribute("crs.action.name", f"xpatching_full_strategy")
        span.set_attribute("service.name", "xpatch_full")
