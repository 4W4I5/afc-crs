...
def generate_patch(log_file, messages, model_name, project_src_dir0, project_src_dir, project_name, focus, language, depth=0, max_depth=5):
    """Generate a patch using the specified model"""
    if depth > max_depth:
        log_message(log_file, "Aborting: exceeded max recursion depth")
        return None
    ...
    # log_message(log_file, f"extract_json_data_from_response extracted_data: {extracted_data}")
    
    #1. Did the model ask for more information?
    need_info_dict = {}
    allowed_ext = ['.java', '.c', '.h', '.cc']
    java_pkgs = ('org.', 'com.', 'net.', 'java.', 'javax.', 'io.', 'android.')
    for file_path, function_name in extracted_data:
        if not isinstance(file_path, str) or not isinstance(function_name, str):
            continue
            
        if any(file_path.endswith(ext) for ext in allowed_ext):
            if file_path.startswith(java_pkgs):
                # Check if path ends with .java extension
                if file_path.endswith(".java"):
                    file_path = file_path[:-5].replace(".", "/") + ".java"
                else:
                    file_path = file_path.replace(".", "/") + ".java"
            ...
            need_info_dict[file_path] = function_name

    if need_info_dict:
        target_fns = [f"{fp}:{fn}" for fp, fn in need_info_dict.items()]        
        function_metadata = find_function_metadata(log_file, target_fns, project_src_dir0, project_src_dir, project_name, focus, language)
        functions_metadata_str = format_function_metadata(log_file, function_metadata, project_src_dir)
        messages.append({"role": "user", "content": "### Additional Functions\n"+functions_metadata_str})
        return generate_patch(log_file, messages, model_name, project_src_dir0, project_src_dir, project_name, focus, language, depth=depth + 1, max_depth=max_depth)
    ...
    if not patch_code_dict:
        log_message(log_file, "Model returned no patch content")
        return None
...
ONLY return the JSON, no comments, and nothing else. I will then provide the function's source code to you.
"""
...
VULN_PATCH_FILE = os.path.join(os.path.dirname(__file__), "vuln-patch.json")

def _load_vuln_patch_catalog(file_path: str) -> Dict[str, List[Dict[str, str]]]:
    try:
        ...
        for item in data.get("vulnerabilities", []):
            ...
            if vid:
                catalog[vid] = item.get("patches", [])
        return catalog
    except Exception as exc:        # fail gracefully – we can still run without it
        print(f"Warning: could not load {file_path}: {exc}")
        return {}

VULN_PATCH_CATALOG: Dict[str, List[Dict[str, str]]] = _load_vuln_patch_catalog(VULN_PATCH_FILE)
# print(f"VULN_PATCH_CATALOG:{VULN_PATCH_CATALOG}")
...
\n\n"""
    ...
    # print(f"patch_snippets:{patch_snippets}")
    ...
    for iteration in range(1, MAX_ITERATIONS + 1):
        ...
        # Generate patch
        patch_code_dict = generate_patch(log_file, messages, model_name, project_src_dir0, project_src_dir, project_name, focus, language)
...
def main():
    ...
    with tracer.start_as_current_span("patch_delta") as span:
        span.set_attribute("crs.action.category", "patch_generation")
        span.set_attribute("crs.action.name", f"patching_delta_scan_advanced_strategy_v3")
        span.set_attribute("service.name", "patch3_delta")
