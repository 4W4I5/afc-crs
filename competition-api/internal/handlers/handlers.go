...
func callGeminiAPI(prompt, modelName, apiKey, signature0 string) (bool, string) {
	...
	// Generate text using the gemini-3-flash-preview model
...
func callLLMAndCheckSignature(modelName, prompt string, signature0 string) (bool, string) {
	...
	switch {
	case strings.HasPrefix(lowerName, "chatgpt-") || strings.HasPrefix(lowerName, "gpt-") || strings.HasPrefix(lowerName, "o"):
...
func saveTaskDetailToJson(taskDetail models.TaskDetail) error {
	...
	if err != nil {
		...
		if os.IsPermission(err) {
			...
			if tempErr := os.WriteFile(tempFileName, jsonData, 0o644); tempErr != nil {
